...
// ═══ TEMPLE BELL — plays MP3 from /temple-bell.mp3 ═══
const templeBellAudioRef={current:null};
function playTempleBell(){
  try{
    if(templeBellAudioRef.current){templeBellAudioRef.current.pause();templeBellAudioRef.current.currentTime=0;}
...
export default function MokshaPatam108(){
  ...
  const doRoll=useCallback((autoRoll=false)=>{
    ...
    // ═══ GURU ENCOUNTER (every 8th turn PER PLAYER) / COSMIC CARD (every 12th) ═══
    const isHumanTurn=!players[cur]?.cpu;
    // Per-player turn counter — each player gets their OWN guru cycle
    if(!gameStats.current.playerTurns)gameStats.current.playerTurns={};
    if(isHumanTurn){
      gameStats.current.playerTurns[cur]=(gameStats.current.playerTurns[cur]||0)+1;
      gameStats.current.humanTurns=(gameStats.current.humanTurns||0)+1;
    }
    const myTurns=gameStats.current.playerTurns[cur]||0;
    const totalTurns=(gameStats.current.turns||0)+1;
    if(isHumanTurn&&!autoRoll){
      // GURU — every 8th of THIS PLAYER's turns
      if(myTurns>1&&myTurns%8===0&&lastKnowledgeTurnRef.current!==`${cur}_${myTurns}`){
  ...
  //    component has 32+ state hooks; without this, every setState on
  ...
  const boardSquares=useMemo(()=>board.map(({num,sn,ld,dl,mk,templeKey,tmpl})=>{
    ...
    else if(tmpl){bg=`linear-gradient(160deg,${tmpl.color}35,${tmpl.color}10 50%,${tmpl.color}20)`;bdr=`${tmpl.color}70`}
    ...
    return(<div key={num} onMouseEnter={()=>!isMobile&&setHov(num)} onMouseLeave={()=>!isMobile&&setHov(null)} onClick={()=>{
      ...
      {/* Font sizes: web/iOS keep the ORIGINAL larger values — they have
          plenty of room on wider screens. Android WebView uses tighter
          sizes because long Skt/EN words (पाइथागोरस, दम्भ-ईर्ष्या, ABHYASA)
          were overflowing narrow mobile squares. overflow+ellipsis guard
          is kept on ALL platforms as a zero-visual-impact safety net. */}
      <span style={{position:"absolute",top:1,left:2,fontSize:IS_ANDROID?"clamp(6px,1vw,9px)":"clamp(7px,1.2vw,11px)",color:"rgba(240,210,130,.5)",fontFamily:"'Noto Serif Devanagari',serif",fontWeight:700}}>{num}</span>
      {mk&&<span style={{fontSize:IS_ANDROID?"clamp(12px,2.2vw,20px)":"clamp(14px,2.5vw,22px)",animation:"mp 3s ease infinite",color:"#f0d050"}}>ॐ</span>}
      {sn&&<><span style={{fontSize:IS_ANDROID?"clamp(9px,1.7vw,14px)":"clamp(10px,2vw,16px)",lineHeight:1}}>𓆙</span><span style={{fontSize:IS_ANDROID?"clamp(6px,1vw,9px)":"clamp(7px,1.2vw,11px)",color:"#ffb040",fontFamily:"'Noto Serif Devanagari',serif",fontWeight:900,lineHeight:1.1,textShadow:"0 0 8px #000,0 1px 4px #000,0 0 12px rgba(180,60,20,.5)",maxWidth:"100%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",padding:"0 1px"}}>{sn.skt}</span><span style={{fontSize:IS_ANDROID?"clamp(4px,.7vw,7px)":"clamp(5px,.9vw,8px)",color:"#ffa040",fontFamily:"'Cinzel',serif",fontWeight:700,lineHeight:1.1,textShadow:"0 0 6px #000,0 0 10px rgba(180,60,20,.4)",maxWidth:"100%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",padding:"0 1px"}}>{sn.en}</span></>}
      {ld&&<><span style={{fontSize:IS_ANDROID?"clamp(8px,1.5vw,12px)":"clamp(9px,1.8vw,14px)",lineHeight:1}}>🪔</span><span style={{fontSize:IS_ANDROID?"clamp(6px,1vw,9px)":"clamp(7px,1.2vw,11px)",color:"#ffe070",fontFamily:"'Noto Serif Devanagari',serif",fontWeight:900,lineHeight:1.1,textShadow:"0 0 8px #000,0 0 12px rgba(200,160,60,.4)",maxWidth:"100%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",padding:"0 1px"}}>{ld.skt}</span><span style={{fontSize:IS_ANDROID?"clamp(4px,.7vw,7px)":"clamp(5px,.9vw,8px)",color:"#f0d060",fontFamily:"'Cinzel',serif",fontWeight:700,lineHeight:1.1,textShadow:"0 0 6px #000",maxWidth:"100%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",padding:"0 1px"}}>{ld.en}</span></>}
      {tmpl&&<><TempleIcon templeKey={templeKey} size={isMobile?(IS_ANDROID?24:28):52} color={tmpl.color}/><span style={{fontSize:IS_ANDROID?"clamp(4px,.85vw,9px)":"clamp(5px,1vw,10px)",color:tmpl.color,fontFamily:"'Noto Serif Devanagari',serif",fontWeight:900,textShadow:`0 1px 3px #000, 0 0 10px ${tmpl.color}50`,lineHeight:1,letterSpacing:0,marginTop:-2,maxWidth:"100%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",padding:"0 1px"}}>{tmpl.name}</span></>}
      {dl&&!tmpl&&<><span style={{fontSize:IS_ANDROID?"clamp(7px,1.3vw,11px)":"clamp(8px,1.5vw,13px)",lineHeight:1}}>⚖</span><span style={{fontSize:IS_ANDROID?"clamp(4px,.7vw,6px)":"clamp(5px,.8vw,7px)",color:"#c8a0f0",fontFamily:"'Cinzel',serif",fontWeight:900,textShadow:"0 0 8px #000",letterSpacing:1}}>DHARMA</span></>}
    </div>);
    ...
    <div style={{...PG,
      ...
      {templeInfo&&(
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:256,
          ...
          <div style={{
            ...
            border:`1px solid ${templeInfo.temple.color}35`,borderRadius:14,
            padding:isMobile?"20px 16px":"28px 24px",textAlign:"center",
            animation:"grahaZoomIn .5s cubic-bezier(0.34,1.52,0.64,1) forwards",
            boxShadow:`0 0 60px ${templeInfo.temple.color}12, 0 20px 50px rgba(0,0,0,.5)`,
          }} onClick={e=>e.stopPropagation()}>
            {/* Temple icon */}
            <div style={{marginBottom:12,display:"flex",justifyContent:"center"}}>
              <TempleIcon templeKey={templeInfo.templeKey} size={isMobile?56:72} color={templeInfo.temple.color}/>
            </div>

            {/* Sanskrit name */}
            <div style={{fontFamily:"'Yatra One',serif",fontSize:isMobile?22:28,
              color:templeInfo.temple.color,letterSpacing:3,marginBottom:4,
              textShadow:`0 0 30px ${templeInfo.temple.color}50`,
            }}>{templeInfo.temple.name}</div>

            {/* English name */}
            <div style={{fontSize:isMobile?12:14,color:"rgba(255,255,255,.4)",
              fontFamily:"'Cinzel',serif",letterSpacing:3,marginBottom:4}}>
            ...
            {/* Divider */}
            <div style={{width:50,height:1,margin:"0 auto 12px",
              background:`linear-gradient(90deg,transparent,${templeInfo.temple.color},transparent)`}}/>

            {/* Shloka */}
            {templeInfo.temple.shloka&&(
              <div style={{marginBottom:14}}>
                <div style={{fontFamily:"'Noto Serif Devanagari',serif",fontSize:isMobile?13:15,
                  color:`${templeInfo.temple.color}aa`,fontStyle:"italic",lineHeight:1.8}}>
                  "{templeInfo.temple.shloka}"
                </div>
                <div style={{fontSize:isMobile?9:10,color:"rgba(200,180,140,.35)",
                  fontFamily:"'Cinzel',serif",letterSpacing:1,marginTop:3}}>
                  {templeInfo.temple.shlokaEn}
                </div>
              </div>
            )}

            {/* Lore */}
            <div style={{fontSize:isMobile?12:14,color:"rgba(200,180,140,.6)",lineHeight:1.9,
      ...
      {templeQuiz&&(
        <div style={{position:"fixed",inset:0,zIndex:260,
          background:`radial-gradient(ellipse at 50% 30%,${templeQuiz.temple.color}15,rgba(0,0,0,.95) 65%)`,
        ...
        <div style={{position:"fixed",inset:0,zIndex:265,overflow:"hidden",
          ...
          {guruEncounter.phase==='intro'&&(
            <div style={{display:"flex",flexDirection:isMobile?"column":"row",
              ...
              <div style={{order:isMobile?1:2,flexShrink:0,
                width:isMobile?"min(55vw,200px)":"min(40vw,480px)",
                display:"flex",flexDirection:"column",alignItems:"center"}}>
          ...
          {guruEncounter.phase==='question'&&(
            <div style={{display:"flex",flexDirection:isMobile?"column":"row",
              ...
              <div style={{flex:1,order:isMobile?2:2,maxWidth:isMobile?"100%":460}}>
                {/* Guru identity row */}
                <div style={{display:"flex",alignItems:"center",gap:isMobile?8:12,
                  marginBottom:isMobile?12:18}}>
                  <div style={{width:isMobile?32:40,height:isMobile?32:40,borderRadius:"50%",
                    overflow:"hidden",border:`1.5px solid ${guruEncounter.guru.color}40`,flexShrink:0}}>
                    {guruEncounter.guru.image?
                      <img src={guruEncounter.guru.image} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                      :<div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:isMobile?18:22,background:`${guruEncounter.guru.color}15`}}>{guruEncounter.guru.icon}</div>
                    }
                  </div>
                  <div>
                    <div style={{fontFamily:"'Yatra One',serif",fontSize:isMobile?16:20,
                      color:guruEncounter.guru.color,letterSpacing:1,lineHeight:1.2}}>{guruEncounter.guru.name}</div>
                    <div style={{fontSize:isMobile?9:10,color:`${guruEncounter.guru.color}50`,
                      fontFamily:"'Cinzel',serif",letterSpacing:1}}>{guruEncounter.guru.en}</div>
                  </div>
                </div>
                ...
                <div style={{
                  padding:isMobile?"16px 14px":"20px 24px",marginBottom:isMobile?16:22,
                  background:`linear-gradient(180deg,${guruEncounter.guru.color}0a,${guruEncounter.guru.color}04)`,
                  border:`1px solid ${guruEncounter.guru.color}22`,
                  borderLeft:`4px solid ${guruEncounter.guru.color}55`,
                ...
                {guruEncounter.correct?(
                  <>
                    <div style={{fontSize:isMobile?40:56,marginBottom:8,
                      ...
                      filter:"drop-shadow(0 0 20px rgba(240,200,80,.5))"}}>🙏</div>
                    <div style={{fontFamily:"'Cinzel',serif",fontSize:isMobile?20:28,
      ...
      {cosmicCard&&(
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,
          ...
          <div style={{
            ...
            padding:"clamp(24px,5vw,36px)",maxWidth:440,width:"min(92vw,calc(100vw-32px))",
            textAlign:"center",animation:"grahaZoomIn .5s cubic-bezier(0.34,1.52,0.64,1) forwards",
