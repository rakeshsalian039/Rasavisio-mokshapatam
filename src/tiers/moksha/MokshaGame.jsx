  ...
  const boardSquares=useMemo(()=>board.map(({num,sn,ld,dl,mk,templeKey,tmpl})=>{
    ...
    return(<div key={num} onMouseEnter={()=>!isMobile&&setHov(num)} onMouseLeave={()=>!isMobile&&setHov(null)} onClick={()=>{
      ...
      {/* Font sizes: web/iOS keep the ORIGINAL larger values. Android
          gets another ~25% reduction on text + a bigger temple icon
          (users wanted the temple to stand out more visually and the
          labels to be less crowded). "DHARMA" text removed on Android
          — the ⚖ icon alone is clear, the word was just clutter.
          overflow+ellipsis safety guards kept everywhere. */}
      <span style={{position:"absolute",top:1,left:2,fontSize:IS_ANDROID?"clamp(5px,.8vw,7px)":"clamp(7px,1.2vw,11px)",color:"rgba(240,210,130,.5)",fontFamily:"'Noto Serif Devanagari',serif",fontWeight:700}}>{num}</span>
