 ...
 * Pure async functions, no React. All Supabase room CRUD + realtime helpers.
 */
...
/**
 * Create a new room and add the host as the first player (seat 0).
 */
...
export async function joinRoom({ roomCode, userId, playerName, charIdx, char }) {
  ...
  if (nextSeat === undefined) throw new Error('Room is full');
...
/**
 * Quick Match — find an open public room or create a new one.
 */
export async function quickMatch({ userId, playerName, charIdx, char }) {
  if (!supabase) throw new Error('Supabase not configured');

  // Find an open public 2-player room that still has space and isn't expired
  ...
  if (rooms) {
    for (const room of rooms) {
      ...
      if (playerCount < room.max_players) {
        // Check we're not already in this room
...
/**
 * Update a player's character selection in the waiting room.
 */
...
/**
 * Start the game — host calls this when all players are ready.
 * Creates the initial game_state row and sets room status to 'active'.
 */
export async function startGame({ roomId, playerCount }) {
  if (!supabase) throw new Error('Supabase not configured');

  const pos = Array(playerCount).fill(1);
  const zeros = Array(playerCount).fill(0);
  const falses = Array(playerCount).fill(false);
...
export async function promoteBotForSlot({ roomId, seatIndex, char, botName = 'Spirit Guide' }) {
  const { error } = await supabase.from('room_players').upsert({
    ...
    player_name: botName,
    char_idx: char?.idx ?? 0,
    char_name: char?.name ?? 'Ganesha',
    char_icon: char?.icon ?? '🐘',
    char_color: char?.color ?? '#c0a030',
    is_bot: true,
    is_ready: true,
    is_connected: true,
...
/**
 * Write the game state after a turn completes (active player calls this).
 */
export async function writeTurnState({ roomId, turnSeq, gameState }) {
  const { error } = await supabase.from('game_state').upsert({
    ...
    cur: gameState.cur,
    pos: gameState.pos,
    punya: gameState.punya,
    papa: gameState.papa,
    shield_a: gameState.shieldA,
    skip_a: gameState.skipA,
    win: gameState.win ?? null,
    dil: gameState.dil ?? null,
    used_dharma: gameState.usedDharma ?? [],
    turn_seq: turnSeq,
    updated_at: new Date().toISOString(),
  });
  if (error) throw error;
...
/**
 * Append a move to the audit log.
 */
...
/**
 * Load the latest game snapshot for reconnect recovery.
 */
export async function loadLatestSnapshot(roomId) {
  const { data } = await supabase
    .from('game_moves')
    .select('snapshot, turn_seq')
    .eq('room_id', roomId)
...
/**
 * Finish the game — write final status.
 */
...
/**
 * Get the count of active seekers across all open rooms (for lobby display).
 */
